...
Texture Assets::loadTextureFromFile(IRenderer& renderer, const string& filename)
{
    ...
    {
        texture.loadSDL(dynamic_cast<RendererSDL&>(renderer), filename); // This type of cast is not elegant nor performant, but we use it on asset loading, at the beginning of the game, so it will do
    }
