	...
	nbVertices(nbVerticesP), nbIndices(nbIndicesP), vertexBuffer(0), indexBuffer(0), vertexArray(0)
{
	...
	glBufferData(GL_ARRAY_BUFFER, nbVertices * 5.0 * sizeof(float), verticesP, GL_STATIC_DRAW);
											 // ^ define the size of the stored data
	...
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(float) * 5, 0); // Vertex coordinates start at index 0
	glEnableVertexAttribArray(1);
	glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(float) * 5, reinterpret_cast<void*>(sizeof(float) * 3)); // UV coordinates start at index 3
}
